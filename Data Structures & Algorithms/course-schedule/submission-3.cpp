...
public:
    ...
    // }
    bool isDirectedCycle(int u,int par,vector<int> &recPath,vector<int> &vis,vector<vector<int>> &adj){
          vis[u]=1;
          recPath[u]=1;

          for(int v: adj[u]){
            if(!vis[v]){
                if(isDirectedCycle(v,u,recPath,vis,adj)){
                    return true;
                }
            }else{ //vis already 
                    if(recPath[v]==1){
    ...
    bool canFinish(int numCourses, vector<vector<int>>& prereq) {
        ...
        for(int i=0;i<numCourses;i++){
            if(!vis[i]){
             if(isDirectedCycle(i,-1,recPath,vis,adj)){
